...
Command.prototype._ptyData = function _ptyData(chunk) {
  ...
  if ((typeof chunk !== "string") && !(chunk instanceof String))
    throw new Error("错误：麻烦输入一个字符串，你在搞事呢？");
...
Command.prototype._exit = function _exit(code, signal) {
  ...
  if ((Date.now() - this.startTime) < 2000 && !signal && code === 0 && !this.interacted) {
    ...
    if (signal)
      this.reply.html("\uD83D\uDC80 通过 %s <strong>强制退出</strong>。", utils.formatSignal(signal));
    else if (code === 0)
      this.reply.html("\u2705 <strong>正常退出</strong>。");
    else
      this.reply.html("\u26D4 <strong>意外退出</strong>，错误码：%s", code);
  }
