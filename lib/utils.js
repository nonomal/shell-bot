...
function createAgent() {
  ...
  try {
    ...
    console.error("错误：解析代理URL时出错： ", e, "Ignoring proxy.");
    return;
  }
  ...
  if ([ "socks:", "socks4:", "socks4a:", "socks5:", "socks5h:" ].indexOf(proxy.protocol) !== -1) {
    try {
      ...
      console.error("错误：加载SOCKS代理支持时出错，请确认正确安装了socks-proxy-agent。已忽略代理。");
      return;
    }
  ...
  if ([ "http:", "https:" ].indexOf(proxy.protocol) !== -1) {
    try {
      ...
      console.error("错误：加载HTTPS代理支持时出错，请确认正确安装了https-proxy-agent。已忽略代理。");
      return;
    }
    return new HttpsProxyAgent(proxy);
  }

  console.error("错误：未知的代理协议： ", util.inspect(proxy.protocol), "已忽略代理。");
}
